//  ORIGINAL CODE - > https://github.com/redpfire/we < -
//
//
#include <Windows.h>
#include <filesystem>
...
int main(int argc, char* argv[])
{
    ...
    {
        //check if valid file. by checking if the file starts with PKGV0001
        prepare_file(argv[2]);
        //read how many files there are in the file
        file_t* files = read_files();
                
        //remove anything after the . so -> scene.pkg --> scene
        //and add "tmp_" tho the front. this will be the folder in which every thing gets dumped
        vector<string> filename_v;
        char* ptr,*e;
        ptr = strtok_s(argv[2], ".",&e);

        string tmppath("tmp_");
        tmppath.append(ptr);

        //create the "tmp_XXXXXX" folder
        bool able = fs::create_directory(tmppath);

        if (!able)
        {
            printf("- unable to create tmp dir (%s)\n", tmppath.c_str());
            return 1;
        }
        ...
        for(int i = 0; i < filecount; ++i)
        {
            ...
            //code from https://www.techiedelight.com/split-a-string-into-a-vector-in-cpp/
            string tmp_token;
            
            stringstream ss(name);
            while (getline(ss, tmp_token, '/')) {
            ...
            if (f == NULL) {
                printf("---> unable to create (%s)\tskipping it <---\n",path.c_str());
                continue;
            }
